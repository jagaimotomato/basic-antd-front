<template>
  <a-card :bordered="false">
    <div v-action:query_dictdata class="table-page-search-wrapper" style="margin-bottom: 15px">
    ...
    >
      ...
      >
        ...
        >
          <a-input
            placeholder="请输入字典值(example:'add')"
            :disabled="disabled"
            v-model="form.value"
          />
        </a-form-model-item>
...
<script>
  ...
  import { getDictDataPage, createDictData, updateDictData, getDictData, delDictData, getDictTypeList } from '@/api/system/dict'
  ...
  export default {
    ...
    data () {
      return {
        ...
        loadData: parameter => {
          return getDictDataPage(Object.assign(parameter, this.queryParam)).then(res => {
    ...
    methods: {
      ...
      handleEdit (record) {
        this.reset()
        const dictCode = record.dictCode || this.selectedRowKeys
      ...
      /** 删除按钮操作 */
      handleDel (record) {
        const dictIds = record.dictCode || this.selectedRowKeys
        const _self = this
        this.$confirm({
          title: '提示',
          content: '是否确认删除字典数据编号为"' + dictIds + '"的数据项?',
          onOk () {
      ...
      handleStatus (checked, record) {
        ...
        this.$confirm({
          title: '提示',
          content: '确认要' + text + 'ID为"' + record.dictCode + '"字典数据吗?',
          onOk () {
            return new Promise((resolve, reject) => {
              _self.form.status = record.status === '1' ? '0' : '1'
              _self.form.dictCode = record.dictCode
              updateDictData(_self.form).then(res => {
                if (res.code === 200) {
