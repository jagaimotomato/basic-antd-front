...
const constantRouterComponents = {
  ...
  // system
  'Permission': () => import('@/views/system/permission/Permission'),
  'Role': () => import('@/views/system/role/Role'),
  'User': () => import('@/views/system/user/User'),
  'Department': () => import('@/views/system/department/Department'),
  ...
  return routerMap.map(item => {
    ...
    const currentRouter = {
      ...
      meta: {
        ...
        hiddenHeaderContent: item.hiddenHeaderContent === 1,
        target: item.target,
...
/* const listToTree = (list, tree, parentId) => {
  list.forEach(item => {
    ...
    if (item.parentId === parentId) {
      ...
      listToTree(list, child.children, item.permissionId)
      // 删掉不存在 children 值的属性
